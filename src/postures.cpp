...
pin::SE3 BipIK::computeBase(const xyzVector &com, const pin::SE3 &leftFoot, const pin::SE3 &rightFoot) {
  ...
  return computeBase(com, Eigen::AngleAxisd((leftYawl + rightYawl) / 2, xyzVector(0, 0, 1)).toRotationMatrix());
}

void BipIK::solve(const pin::SE3 &base, const pin::SE3 &leftFoot, const pin::SE3 &rightFoot, const Eigen::VectorXd &q0,
...
void BipIK::solve(const xyzVector &com, const pin::SE3 &leftFoot, const pin::SE3 &rightFoot, const Eigen::VectorXd &q0,
                  Eigen::VectorXd &posture) {
  pin::SE3 base = computeBase(com, leftFoot, rightFoot);
  solve(base, leftFoot, rightFoot, q0, posture);
}

void BipIK::solve(const xyzVector &com, const RotMatrix &baseRotation, const pin::SE3 &leftFoot,
                  const pin::SE3 &rightFoot, const Eigen::VectorXd &q0, Eigen::VectorXd &posture) {
...
Eigen::Vector2d BipIK::computeCoP(pin::Data &data, const Eigen::VectorXd &posture, const Eigen::VectorXd &velocity,
                                  const Eigen::VectorXd &acceleration, const Wrench &externalWrench,
...
legJoints LegIG::solve(const pin::SE3 &base, const pin::SE3 &endEffector) {
  ...
  double cos_q5 = (pow(distanceHipAnkle, 2) - pow(info.femurLength, 2) - pow(info.tibiaLenght, 2)) /
...
Eigen::VectorXd BipedIK::computePosture(const pin::SE3 &com, const pin::SE3 &leftFoot, const pin::SE3 &rightFoot,
                                        Eigen::VectorXd posture) {
...
void BipedIK::computeJointDerivatives(const Eigen::VectorXd &posture1, const Eigen::VectorXd &posture2,
