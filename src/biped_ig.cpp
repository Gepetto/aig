...
void BipedIG::initialize(const BipedSettings &settings) {
  ...
  pinocchio::srdf::loadReferenceConfigurations(model_, settings_.srdf_path, false);
...
void BipedIG::configurateLegs() {
  ...
  pinocchio::JointIndex right_hip_id = model_.getJointId(settings_.right_hip_joint_name);
  pinocchio::JointIndex right_knee_id = model_.getJointId(settings_.right_knee_joint_name);
  pinocchio::JointIndex right_ankle_id = model_.getJointId(settings_.right_ankle_joint_name);
  ...
  right_leg_settings.ankle_from_foot = -model_.frames[rightSoleID].placement.translation();
  right_leg_.initialize(right_leg_settings);
}

pinocchio::SE3 BipedIG::computeBase(const Eigen::Vector3d &com, const Eigen::Matrix3d &baseRotation) {
  pinocchio::SE3 base;
...
pinocchio::SE3 BipedIG::computeBase(const Eigen::Vector3d &com, const pinocchio::SE3 &leftFoot,
  ...
  return computeBase(com, Eigen::AngleAxisd((leftYawl + rightYawl) / 2, Eigen::Vector3d(0, 0, 1)).toRotationMatrix());
}

void BipedIG::solve(const pinocchio::SE3 &base, const pinocchio::SE3 &leftFoot, const pinocchio::SE3 &rightFoot,
...
void BipedIG::solve(const Eigen::Vector3d &com, const pinocchio::SE3 &leftFoot, const pinocchio::SE3 &rightFoot,
                    const Eigen::VectorXd &q0, Eigen::VectorXd &posture) {
  pinocchio::SE3 base = computeBase(com, leftFoot, rightFoot);
  solve(base, leftFoot, rightFoot, q0, posture);
}

void BipedIG::solve(const Eigen::Vector3d &com, const Eigen::Matrix3d &baseRotation, const pinocchio::SE3 &leftFoot,
...
void BipedIG::derivatives(const Eigen::VectorXd &q1, const Eigen::VectorXd &q3, Eigen::VectorXd &posture,
...
void BipedIG::solve(const std::array<pinocchio::SE3, 3> &bases, const std::array<pinocchio::SE3, 3> &leftFeet,
                    const std::array<pinocchio::SE3, 3> &rightFeet, const Eigen::VectorXd &q0,
                    Eigen::VectorXd &posture, Eigen::VectorXd &velocity, Eigen::VectorXd &acceleration,
                    const double &dt) {
...
void BipedIG::solve(const std::array<Eigen::Vector3d, 3> &coms, const std::array<pinocchio::SE3, 3> &leftFeet,
                    const std::array<pinocchio::SE3, 3> &rightFeet, const Eigen::VectorXd &q0,
                    Eigen::VectorXd &posture, Eigen::VectorXd &velocity, Eigen::VectorXd &acceleration,
                    const double &dt) {
  Eigen::VectorXd q1, q3;
...
void BipedIG::solve(const std::array<Eigen::Vector3d, 3> &coms, const std::array<Eigen::Matrix3d, 3> &baseRotations,
...
Eigen::Vector2d BipedIG::computeCoP(const Eigen::VectorXd &posture, const Eigen::VectorXd &velocity,
                                    const Eigen::VectorXd &acceleration,
                                    const Eigen::Matrix<double, 6, 1> &externalWrench, bool flatHorizontalGround) {
  // The external wrench is supposed to be expressed in the frame of the root link.

  Eigen::Matrix<double, 6, 1> tauMw = pinocchio::rnea(model_, data_, posture, velocity, acceleration).head(6);
  Eigen::Vector3d groundTorqueMo =
      tauMw.tail(3) - externalWrench.tail(3) +
      pinocchio::skew(Eigen::Vector3d(posture.head(3))) * (tauMw.head(3) - externalWrench.head(3));
...
Eigen::Vector2d BipedIG::computeCoP(const Eigen::VectorXd &posture, const Eigen::VectorXd &velocity,
  ...
  Eigen::Matrix<double, 6, 1> externalWrench = Eigen::Matrix<double, 6, 1>::Zero();
  return computeCoP(posture, velocity, acceleration, externalWrench, flatHorizontalGround);
}
